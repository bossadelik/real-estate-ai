...
serve(async (req) => {
  if (req.method !== "POST") return new Response("Solo POST ammesso", { status: 405 });

  const form = await req.formData();
  const file = form.get("image") as File;
  if (!file) return new Response("Nessuna immagine", { status: 400 });
  ...
  if (!b64) return new Response("Enhancement fallito", { status: 500 });
